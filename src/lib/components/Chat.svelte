<script>
    ...
    let system_message = $state([
            {
                role: "system",
                content: "You are a helpful retrieval augmented generation assistant who outputs in markdown format. You may use the notes for extra knowledge but do not let the user think they directly shared them"
            },
        ]);
...
<div class="flex flex-col w-full h-full">
    ...
    <Card.Root>
        <Card.Content class="flex flex-row items-center gap-2">
            <Textarea bind:value={prompt_input} class="resize-none" onkeydown={(e) => {

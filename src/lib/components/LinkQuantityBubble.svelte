<script>
    ...
    import { getLinksAndQuantity } from '$lib/functions/dbfuncs';
    import * as d3 from 'd3';
    import * as d3force from 'd3-force';

    let links_and_quantity;
    let simulation;
    let graph_element;
    let graph_w;
    let graph_h;
    ...
    onMount(() => {
        ...
        ro = new ResizeObserver(entries => {
            for (let entry of entries) {
                ...
                if (links_and_quantity) runSimulation();
            }
        });
    ...
    async function loadData() {
        links_and_quantity = Object.values(await getLinksAndQuantity());
        if (graph_w && graph_h) runSimulation();
    }

    async function runSimulation() {
        const quantities = links_and_quantity.map(d => d.quantity);
        const minQ = d3.min(quantities);
        const maxQ = d3.max(quantities);
        const rScale = d3.scaleSqrt().domain([minQ, maxQ]).range([30, 80]);

        simulation = d3force.forceSimulation(links_and_quantity)

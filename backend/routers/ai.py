...
generation_model_name = "unsloth/Qwen3-1.7B-GGUF"
generation_file_name = "Qwen3-1.7B-Q8_0.gguf"
...
def getEmbeddingModel():
    ...
    if embedding_model is None:
        # installs torch separately; transformers does *not* bundle it
        # feature-extraction gives you a mean-pooled vector
        embedding_model = LlamaCppEmbeddings(model_path=str(settings["app_data_dir"])+"/models/"+embedding_file_name)
...
def getGenerationModel():
    ...
    if generation_model is None:
        generation_model = ChatLlamaCpp(model_path=str(settings["app_data_dir"]) + "/models/" + generation_file_name)
